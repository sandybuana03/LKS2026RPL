...
<div class="content-wrapper pt-8 px-6 bg-slate-50">
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">Manajemen Guru</h1>
            <p class="text-slate-500">Mapping pengajar ke kelas yang diampu.</p>
        </div>
        <button data-toggle="modal" data-target="#addGuru" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100 transition-all">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-6 border border-white overflow-hidden">
        <table class="table table-borderless align-middle datatable">
            ...
            <tbody>
                <?php
                $q = mysqli_query($conn, "SELECT users.*, kelas.nama_kelas FROM users LEFT JOIN kelas ON users.id_kelas = kelas.id_kelas WHERE role='guru'");
                ...
                <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="text-center">
                        <a href="proses_user.php?aksi=hapus&id=<?= $d['id_user'] ?>" class="text-red-400 hover:text-red-600 p-2" onclick="return confirm('Hapus guru ini?')"><i class="fas fa-trash-alt"></i></a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-[2.5rem] border-none shadow-2xl overflow-hidden">
            <div class="bg-indigo-600 p-8 text-white">
                <h3 class="text-2xl font-black">Tambah Guru</h3>
                <p class="text-indigo-100 text-sm">Daftarkan pengajar baru ke sistem.</p>
            </div>
            <form action="proses_user.php?aksi=tambah_guru" method="POST" class="p-8 space-y-4">
